    ...
    public void initialize() throws FileNotFoundException {
        ...
        for (Group group : Database.groups) {
            if (group.members.contains(Database.users.get(v))) {
                ...
                forwardGroup.setOnMouseClicked(mouseEvent -> {
                    try {
                        GroupMessage newGroupMessage = new GroupMessage(Database.users.get(v), "(Forwarded) " + readFile(new File("D:\\forwardGroupMessage")), group);
        ...
        for (Direct direct : Database.directs){
            ...
            forwardDirect.setOnMouseClicked(mouseEvent -> {
                try {
                    DirectMessage newDirectMessage = new DirectMessage(Database.users.get(v),"(Forwarded) "+readFile(new File("D:\\forwardGroupMessage")),direct);
                } catch (FileNotFoundException e) {
                    e.printStackTrace();
                }
            });
